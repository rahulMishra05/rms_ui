...
const {id} = useParams();
    ...
    useEffect(() => {
        fetch('https://localhost:44396/api/SkillMaster/GetActiveSkill', {
            method: 'GET',
            headers: {
                    ...
                    <tbody>
                        {
                            result.map(items => {
                                ...
                                return (
                            <tr key={0}>
                                <td><input {...register('skill')} type="checkbox" name='skill' value={items.skillName+"::"+items.skillCategory} 
                                defaultChecked={editData.skillList ? editData.skillList[0].skillName:null}/></td>
