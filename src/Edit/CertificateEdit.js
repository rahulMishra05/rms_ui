...
function CertificationTraining() {
 ...
 const {id}=useParams();
  ...
  const customFunction = (d) => {
    ...
    axios.put(`https://localhost:7258/api/Resume/${id}`, CertiObj);
  }
  const [editData,setEditData]=useState([]);
  useEffect(()=>{
  axios.get(`https://localhost:44396/api/Resumes/${id}`). then((res)=>setEditData(res.data))
},[])
let certificateArray = editData.certifications?editData.certifications[0].certificationName.split(","):null;
let trainingArray = editData.trainings?editData.trainings[0].trainingname.split(","):null;
      ...
      <form onSubmit={handleSubmit((data) => customFunction(data))} className="formCertification">
        ...
        <div className="bulletPoints row">
          ...
          <div>
            ...
            {certificateArray ? certificateArray.map((item)=>{
